...
convert_dxbc_vertex_hull_shader(
  ...
  auto binding_map = rootsig_vs || rootsig
                         ? setup_binding_rootsig(
                               &vertex_shader_info, func_signature, module, D3D10_SB_VERTEX_SHADER,
                               (rootsig_vs ? rootsig_vs : rootsig)->bytecode,
                               (rootsig_vs ? rootsig_vs : rootsig)->bytecode_length,
                               SM50_BINDING_INDEX_ROOT_ARGUMENTS, SM50_BINDING_INDEX_STATIC_SAMPLERS
                           )
